      ...
      <div className="relative bg-white dark:bg-[#1a1a2e] rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-md sm:mx-4 overflow-hidden transform transition-all animate-slide-up">
          ...
          <div className="flex gap-3 px-6 pb-6">
            <button
              onClick={onClose}
              disabled={isLoading}
              className="flex-1 px-4 py-1 text-xs font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-800 rounded-xl hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors disabled:opacity-50"
            >
              {cancelText}
            </button>
            <button
              onClick={onConfirm}
              disabled={isLoading}
              className={`flex-1 px-4 py-1 text-xs font-medium rounded-xl transition-all disabled:opacity-50 ${styles.button}`}
            >
              {isLoading ? (
