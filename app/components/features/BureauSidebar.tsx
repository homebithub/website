...
export default function BureauSidebar() {
  ...
  return (
    <aside className="w-full sm:w-56 bg-white dark:bg-[#13131a] border-r border-gray-100 dark:border-purple-500/20 p-4 rounded-xl shadow-sm dark:shadow-glow-sm flex flex-col space-y-2 transition-colors duration-300">
      {menu.map((item) => (
        <Link
          key={item.to}
          to={item.to}
          prefetch="intent"
          className={`block px-4 py-1 rounded-xl text-sm font-medium transition-all duration-150
