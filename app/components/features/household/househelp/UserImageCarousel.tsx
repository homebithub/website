...
export default function UserImageCarousel({ images, carouselIdx, setCarouselIdx }: UserImageCarouselProps) {
  if (!images || images.length === 0) {
    return (
      <div className="w-full h-48 flex items-center justify-center bg-gray-100 dark:bg-slate-800 rounded-lg">
        <span className="text-gray-500 dark:text-gray-300">No images uploaded.</span>
      </div>
    );
  }
  ...
  return (
    <div className="w-full flex flex-col items-center">
      <div className="relative w-full h-48 flex items-center justify-center bg-gray-100 dark:bg-slate-800 rounded-lg mb-2">
        <img
          src={images[carouselIdx]?.url || images[carouselIdx]}
