...
export function HiringCardModal({ open, onClose, eyebrow, title, imageUrl, initials, status, summary, fields, details, message, actions }: HiringCardModalProps) {
  ...
  return (
    <div className="hb-mobile-modal-viewport fixed inset-0 z-[120] flex items-end justify-center p-0 sm:items-center sm:p-5" role="dialog" aria-modal="true" aria-label={`${title} details`}>
      ...
      <section className="hb-mobile-modal-panel relative w-full overflow-y-auto rounded-t-3xl border border-purple-500/30 bg-white pb-[env(safe-area-inset-bottom,0px)] shadow-2xl dark:bg-[#13131a] sm:max-w-2xl sm:rounded-3xl">
        <header className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-purple-100 bg-white/95 px-5 py-4 backdrop-blur dark:border-purple-500/30 dark:bg-[#13131a]/95 sm:px-7">
        ...
        <div className="space-y-5 px-5 py-5 sm:px-7 sm:py-6">
          {summary && <div className="rounded-2xl bg-purple-50 p-4 text-sm text-gray-700 dark:bg-purple-500/10 dark:text-purple-100">{summary}</div>}
          <dl className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            {fields.filter((field) => field.value !== undefined && field.value !== null && field.value !== '').map((field) => (
              <div key={field.label} className="rounded-2xl border border-purple-100 p-3 dark:border-purple-500/30">
          ...
          {message && <div className="rounded-2xl border border-purple-100 p-4 dark:border-purple-500/30"><p className="mb-1 text-[10px] font-bold uppercase tracking-wide text-purple-500">Message / notes</p><div className="whitespace-pre-wrap text-sm text-gray-700 dark:text-purple-100">{message}</div></div>}
        </div>
        {actions && <footer className="flex flex-col gap-2 border-t border-purple-100 bg-white px-5 py-4 dark:border-purple-500/30 dark:bg-[#13131a] sm:flex-row sm:flex-wrap sm:justify-end sm:px-7 [&>button]:w-full sm:[&>button]:w-auto">{actions}</footer>}
      </section>
    </div>
  );
}
