...
export default function SupportChat() {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (!open && data.messages.length > previousCount.current) setUnread((n) => n + data.messages.slice(previousCount.current).filter((m) => m.sender_type === 'agent').length);
  ...
  useEffect(() => { refresh(); if (!chat) return; const id = window.setInterval(refresh, 3000); return () => window.clearInterval(id); }, [chat?.id, refresh]);
  ...
  return <>
    ...
    {open && <section aria-label="Homebit Support chat" className="fixed inset-3 z-[70] flex flex-col overflow-hidden rounded-2xl border border-purple-200 bg-white shadow-2xl dark:border-purple-500/30 dark:bg-[#13131a] sm:inset-auto sm:bottom-6 sm:right-6 sm:h-[620px] sm:w-[390px]">
      ...
      {!chat ? <div className="flex flex-1 flex-col justify-center space-y-4 p-5">
        ...
        <div className="border-b border-purple-100 px-4 py-2 text-xs text-gray-500 dark:border-purple-500/20 dark:text-gray-400">Ticket #{chat.ticket_number} · <span className="capitalize">{chat.status}</span></div>
        <div className="flex-1 space-y-3 overflow-y-auto p-4">{messages.map((m) => { const mine=m.sender_type==='customer'; return <div key={m.id} className={`group flex ${mine?'justify-end':'justify-start'}`}><div className="max-w-[82%]"><div className={`rounded-2xl px-3 py-2 text-sm ${mine?'rounded-br-md bg-gradient-to-r from-purple-600 to-pink-600 text-white':'rounded-bl-md bg-purple-50 text-gray-800 dark:bg-white/10 dark:text-gray-100'}`}><p className="whitespace-pre-wrap break-words">{m.body}</p>{m.attachment_url&&<a className="mt-1 block underline" target="_blank" rel="noreferrer" href={m.attachment_url}>📎 {m.attachment_name||'Attachment'}</a>}</div><div className={`mt-1 flex gap-1 ${mine?'justify-end':''}`}><button onClick={()=>setReplyTo(m)} title="Reply" className="opacity-0 group-hover:opacity-100"><Reply className="h-3.5 w-3.5"/></button>{QUICK_REACTIONS.map(e=><button key={e} onClick={async()=>{await supportService.react(m.id,chat.access_token,e);refresh()}} className={`text-xs ${m.reactions?.customer===e?'rounded-full bg-purple-100 px-1':''}`}>{e}</button>)}</div></div></div>})}<div ref={bottomRef}/></div>
