...
export default function SupportChat() {
  ...
  useEffect(() => {
    try {
      ...
      if (saved && Number.isFinite(saved.x) && Number.isFinite(saved.y)) setLauncherPosition(saved);
    } catch {}
  }, []);
  ...
  return <>
    <button ref={launcherRef} style={launcherPosition?{left:launcherPosition.x,top:launcherPosition.y}:{bottom:launcherBottom}} onPointerDown={startLauncherDrag} onPointerMove={moveLauncher} onPointerUp={finishLauncherDrag} onPointerCancel={finishLauncherDrag} onClick={() => {if(!launcherDrag.current?.moved)setOpen(true)}} aria-label="Chat with Homebit Support. Drag to reposition." title="Drag to move · Click to open" className={`fixed z-50 hidden h-14 touch-none select-none items-center gap-2 rounded-full bg-gradient-to-r from-purple-600 to-pink-600 px-5 text-sm font-semibold text-white shadow-2xl shadow-purple-500/40 lg:flex ${launcherPosition?'':'right-6 transition-[bottom,transform] duration-200'} ${launcherDragging?'cursor-grabbing scale-105':'cursor-grab hover:scale-105'}`}>
    ...
    {open && <section aria-label="Homebit Support chat" className="fixed inset-3 z-[70] flex flex-col overflow-hidden rounded-2xl border border-purple-200 bg-white shadow-2xl dark:border-purple-500/30 dark:bg-[#13131a] sm:inset-auto sm:bottom-6 sm:right-6 sm:h-[620px] sm:w-[390px]">
      ...
      {!chat ? <div className="flex flex-1 flex-col justify-center space-y-4 p-5">
        ...
        <input value={name} onChange={(e)=>setName(e.target.value)} maxLength={120} placeholder="Your name" className="rounded-xl border border-purple-200 bg-transparent px-3 py-2.5 text-base dark:border-purple-500/30 sm:text-sm" />
        <input value={email} onChange={(e)=>setEmail(e.target.value)} maxLength={320} type="email" placeholder="Email (optional, for updates)" className="rounded-xl border border-purple-200 bg-transparent px-3 py-2.5 text-base dark:border-purple-500/30 sm:text-sm" />
        <textarea value={draft} onChange={(e)=>setDraft(e.target.value.slice(0, CHAT_MESSAGE_LIMIT))} rows={5} placeholder="Tell us what you need help with…" className="resize-none rounded-xl border border-purple-200 bg-transparent p-3 text-base dark:border-purple-500/30 sm:text-sm" />
