...
export default function HousehelpFindHouseholds() {
  ...
  return (
    <div className="w-full">
      ...
      {(() => {
        const baseClass = 'rounded-3xl bg-gradient-to-r from-purple-600 via-pink-600 to-pink-500 shadow-2xl shadow-purple-500/30 mb-6 transition-all duration-300';
        const paddingClass = filtersExpanded ? 'p-6 sm:p-8' : 'p-5 sm:p-6';
        return (
          <div className={`${baseClass} ${paddingClass}`}>
            <button
              ...
              className="group flex items-center justify-between gap-3 rounded-2xl bg-white/0 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-white/70"
            >
              <span className="flex flex-col text-white">
                <span className="text-lg sm:text-xl font-extrabold">Find Households</span>
                <span className="text-xs sm:text-sm text-white/80 mt-1">Tap to expand and tailor your search filters.</span>
              </span>
              <ChevronDownIcon className={`w-5 h-5 text-white transition-transform ${filtersExpanded ? 'rotate-180' : ''}`} />
            </button>
            {filtersExpanded && (
              <div className="mt-6">
