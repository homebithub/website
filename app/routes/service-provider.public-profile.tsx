...
function normalizeServiceProviderData(raw: any): ServiceProviderData {
  if (!raw) return {} as ServiceProviderData;
...
export default function ServiceProviderPublicProfile() {
  ...
  const [profile, setProfile] = useState<ServiceProviderData | null>(null);
  ...
  useEffect(() => {
    ...
    if (changed) {
      navigate(`/service-provider/public-profile?${nextParams.toString()}`, { replace: true, state: location.state });
    }
  ...
  const currentProfileType: string | undefined = currentUser?.profile_type || getStoredCanonicalProfileType() || undefined;
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        let profileData: any;
        if (profileId) {
          profileData = await resolveServiceProviderProfile(profileId, { identifierType: 'auto' });
        } else {
        ...
        const listingPromise = !queryOpenForWorkId && normalizedProfile.id
          ? openForWorkService.getOpenForWorkByServiceProvider(normalizedProfile.id, '').catch(() => null)
          : Promise.resolve(null);
  ...
  const handleChat = async () => {
    ...
    try {
      ...
      let serviceProviderId = serviceProviderUserId;

      // If a provider reaches this view in a reversed context, keep participant
      // identifiers assigned to the correct side of the conversation.
      if (isServiceProviderProfileType(profileType)) {
        householdId = serviceProviderUserId;
      ...
      const payload: StartConversationPayload = {
        ...
        service_provider_profile_id: targetProfileId,
      };
  ...
  const handleShortlistToggle = async () => {
    if (!shortlistTargetId) {
      setActionError('This service provider has not published an open-for-work listing yet.');
      return;
    }
  ...
  const showOwnerBackButton = !isViewingOther && isServiceProviderProfileType(currentProfileType);
  ...
  if (error || !profile) {
    return <FullPageError title="Service provider profile unavailable" message="We couldn't load this service provider profile. It may have changed, or the connection may have been interrupted." onRetry={() => setRetryKey((value) => value + 1)} backTo={navigationState.backTo || queryBackTo} backLabel={backButtonText} embed={isEmbed} />;
  }
    ...
    <div className="min-h-screen flex flex-col">
                ...
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div className="space-y-3">
                    {shouldShowBackButton && (
                      <button
                        onClick={isViewingOther ? handleBackNavigation : () => navigate('/service-provider/profile')}
                    ...
                    <div>
                      <h1 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-white flex items-center gap-2">
                        {displayName || 'Service Provider Profile'}
                  ...
                  {isViewingOther && (
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3 self-start lg:self-auto">
                      <button
                        ...
                        aria-label={isShortlisted ? 'Unsave service provider' : 'Save service provider'}
      ...
      {isViewingOther && profile && (
        <HireRequestModal
          ...
          serviceProviderName={`${profile.first_name || ''} ${profile.last_name || ''}`.trim()}
        />
      )}
    </div>
  );
