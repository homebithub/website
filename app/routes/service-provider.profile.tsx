...
function normalizeServiceProviderProfileResponse(response: any): ServiceProviderData {
...
export default function ServiceProviderProfile() {
  ...
  const { progress, refetch: refetchProgress } = useOnboardingProgress(userId, 'service_provider');
  const profileCompletionReminder = useProfileCompletionReminder(userId, 'service_provider');
    ...
    const fetchProfile = async () => {
      ...
      try {
        ...
        const [profileResult, docsResult] = await Promise.allSettled([
          grpcProfileService.getCurrentServiceProviderProfile(''),
          documentService.getUserDocuments('', 'profile_photo'),
        ]);

        const normalized = profileResult.status === 'fulfilled'
          ? normalizeServiceProviderProfileResponse(profileResult.value)
          : normalizeServiceProviderProfileResponse({
  ...
  const handleCompleteFeaturePicks = () => {
    rememberProfileCompletionBaseline(`${userId}:service_provider`, progress?.completion_percentage);
    ...
    navigate('/onboarding/features', {
      state: {
        ...
        profileType: 'service_provider',
        returnTo: '/service-provider/profile',
      },
    });
  };
  ...
  const handleDeletePhoto = async () => {
    ...
    try {
      ...
      // Step 1: Find the document by URL via gRPC. A lookup failure can fall
      // back to updating the profile, but a storage deletion failure must stop
      // the operation so a protected document is not hidden only in the UI.
      let document: any;
      try {
        const docsData = await documentService.getUserDocuments('', 'profile_photo');
        const allDocs = docsData?.data || docsData?.documents || docsData || [];
        const documentsArray = Array.isArray(allDocs) ? allDocs : [];
        document = documentsArray.find((doc: any) => (doc.public_url || doc.signed_url || doc.url) === photoUrl);
      } catch (err) {
        console.warn('Failed to fetch documents, will only remove from profile');
      }

      // Step 2: Delete from documents table and S3 via gRPC. In particular,
      // Smile ID selfies are protected by auth; let refusal reach the outer
      // error handler instead of removing the URL from the profile anyway.
      if (document?.id) {
      ...
      try {
        await grpcProfileService.updateServiceProviderFields('', 'service_provider', { photos: updatedPhotos });
      } catch (err) {
    ...
    return (
      <div className="max-w-2xl mx-auto mt-8">
        <div className="p-6 rounded-xl bg-yellow-50 dark:bg-yellow-900/20 border-2 border-yellow-200 dark:border-yellow-500/30">
          ...
          <p className="text-gray-700 dark:text-gray-300 mb-4">You haven't completed your service provider profile yet.</p>
          <button
            onClick={() => setRetryKey((prev) => prev + 1)}
  ...
  return (
    ...
    <div className="max-w-5xl mx-auto px-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <h1 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-white mt-2 flex items-center gap-2">
              {formatDisplayName(profile, undefined, 'Service provider')}
          ...
          <div className="flex items-center gap-2 self-start">
            ...
            <button
              onClick={() => navigate('/service-provider/public-profile')}
      ...
      <ProfileAccountSummary
        ...
        fallbackProfileType="service_provider"
